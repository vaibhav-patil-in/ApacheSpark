...
val backwardShips = shipList.map( (ship: String) => {ship.reverse})
 ...
 var upperCaseShip = shipList.map((ship: String) => {ship.toUpperCase()});
                                                  //> upperCaseShip  : List[String] = List(ENTERPRISE, DEFIANT, VOYAGER, DEEP SPA
                                                  //| CE NINE)
 for(ship <- upperCaseShip) {
 	println(ship);                            //> ENTERPRISE
                                                  //| DEFIANT
                                                  //| VOYAGER
                                                  //| DEEP SPACE NINE
 }
...
val filteredList = numbersList.filter((x: Int) => (x % 3) ==0 )
